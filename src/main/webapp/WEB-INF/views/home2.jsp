...
<script>
	document.addEventListener('DOMContentLoaded', function() {
	  ...
	  document.querySelectorAll('.truncate').forEach(el => truncateText(el, 100));
	});
</script>
<body>
		  ...
		  <c:if test="${not empty weatherAdvice}">
		    <div class="weather-content">
		      ...
		      <div class="weather-text">
		        <!-- <h3 class="weather-title">오늘의 날씨 안내</h3> -->
	    ...
	    <c:if test="${not empty houseworks}">
	      <ul class="list-group list-group-flush">
	        <c:forEach items="${houseworks}" var="work" varStatus="status">
	          <li class="list-group-item truncate">${status.count}. ${work.description}</li>
	        </c:forEach>
	      </ul>
	    </c:if>
	    <c:if test="${empty houseworks}">
	      <div class="empty-message truncate">오늘은 특별한 할 일이 없어요. 가족과 함께 즐거운 시간 보내세요!</div>
	    </c:if>
	  </div>
	</div>
