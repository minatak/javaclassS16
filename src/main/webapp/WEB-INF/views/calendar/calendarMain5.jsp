  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function openEventModal(event, start, end, allDay) {
      ...
      if (event) {
        ...
        $('#detailDate').text(formatDateRange(event.start, event.end, event.allDay));
        $('#detailDescription').text(event.extendedProps.description);
        $('#detailSharing').text(event.extendedProps.sharing ? '공유됨' : '개인');
        $('#detailAuthor').text('작성자: ' + event.extendedProps.memberId);
    ...
    function formatDateRange(start, end, allDay) {
      if (allDay) {
        return moment(start).format('YYYY-MM-DD') + (end ? ' ~ ' + moment(end).format('YYYY-MM-DD') : '');
      } else {
    ...
    function resetForm() {
      ...
      $('#eventSharing').prop('checked', false);
      toggleAllDay();
    }
    ...
    function fillForm(event) {
      ...
      $('#eventSharing').prop('checked', event.extendedProps.sharing);
      toggleAllDay();
    }
   ...
   /*  $('#saveEvent').click(function() {
      ...
      $.ajax({
        url: updatedEvent.idx ? '${ctp}/calendar/calendarUpdate' : '${ctp}/calendar/calendarInput',
        type: 'POST',
        data: updatedEvent,
        success: function(res) {
          if(res === '1') {
            ...
            alert('일정 ' + (updatedEvent.idx ? '수정' : '등록') + '에 실패했습니다. 오류: ' + res);
          }
        },
        error: function(jqXHR, textStatus, errorThrown) {
          console.error("AJAX error: " + textStatus + ' : ' + errorThrown);
    	  ...
    	  $.ajax({
    	    ...
    	    success: function(res) {
    	      if(res === '1') {
    	        $('#eventModal').modal('hide');
    	        calendar.refetchEvents();
    	        showAlert('일정이 ' + (updatedEvent.idx ? '수정' : '등록') + '되었습니다.');
    	      } else {
    	        showAlert('일정 ' + (updatedEvent.idx ? '수정' : '등록') + '에 실패했습니다.');
    	      }
    	    },
    	    error: function(xhr, status, error) {
	  ...
	  <div class="modal-dialog modal-dialog-centered" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title" id="modalTitle">일정 상세</h5>
	      ...
	      <div class="modal-footer">
	        <button type="button" class="modalBtn btnPrimary" id="editEvent">수정</button>
